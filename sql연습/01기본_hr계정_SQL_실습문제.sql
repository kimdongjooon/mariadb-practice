...
select * from salaries where salaries.emp_no =10002; 
...
-- 문제4.
-- 현재 근무하고 있는 직원 수는 몇 명입니까? (salaries 테이블을 사용합니다.) - 현재 to_date='9999-01-01'
select count(distinct emp_no) as '현재 근무 인원수' from salaries where to_date = '9999-01-01';
...
select * from titles;
select count(emp_no) as '사원 수'from titles where title = 'Engineer' ;
select count(distinct emp_no) as '사원 수' from titles where title = 'Engineer';
